	...
	*/
	...
	function getTableList($skip_authentication = false, $include_internal_tables = false) {
		...
		$arrTables = [
			...
			'tenants' => ['Tenants', '', 'table.gif', 'Suggestions &amp; Other Apps'],
			...
			'event_outcomes_expected_table' => ['Outcomes Expected Table', '', 'table.gif', 'Events / Meetings / Goals Apps'],
			'event_participants_table' => ['Participants / Speaker / VIP List - App', '', 'table.gif', 'Events / Meetings / Goals Apps'],
			...
			'meetings_agenda_table' => ['Meeting Agenda - App', '', 'table.gif', 'Events / Meetings / Goals Apps'],
			'meetings_participants_table' => ['Participants / Speaker / VIP List - App', '', 'table.gif', 'Events / Meetings / Goals Apps'],
			'meetings_decision_table' => ['Meeting Decision - App', '', 'table.gif', 'Events / Meetings / Goals Apps'],
			...
			$schema = [
				...
				'event_participants_table' => [
					...
					'event_participants_str' => [
						'appgini' => "TEXT NULL",
						'info' => [
							'caption' => 'Event participants str',
							'description' => '',
						],
					],
				],
	...
	*/
	...
	function get_parent_tables($table) {
		...
		$parents = [
			...
			'event_decision_table' => [
				'event_outcomes_expected_table' => ['outcomes_expected_lookup'],
			...
			'meetings_decision_table' => [
				'meetings_agenda_table' => ['agenda_lookup'],
				'user_table' => ['decision_actor'],
			],
			'visiting_card_table' => [
				'user_table' => ['given_by'],
	...
	function calculated_fields() {
		...
		return [
			...
			'suggestion' => [],
			'event_table' => [
				'event_str' => 'SELECT CONCAT(
					`event_table`.`event_name`,
					\':\',`event_table`.`participants`,
					\':\',`event_table`.`venue`,
					\':\',`event_table`.`event_from_date`
					) FROM %TABLENAME% WHERE `event_table`.`id`=%ID%',
			],
			'event_outcomes_expected_table' => [
				'outcomes_expected_str' => 'SELECT CONCAT(
					    `event_outcomes_expected_table`.`target_audience`,\':\', 
					    `event_outcomes_expected_table`.`expected_outcomes`
					)
					FROM %TABLENAME%
					WHERE `event_outcomes_expected_table`.`id` = %ID%;
					',
			],
			'event_participants_table' => [
				'event_participants_str' => 'SELECT CONCAT(
					`event_participants_table`.`name`,\':\',
					`event_participants_table`.`designation`,\':\',
					`event_participants_table`.`participant_type`,\':\',
					`event_participants_table`.`accepted_status`,\':\',
					`event_participants_table`.`status_date`
					) FROM %TABLENAME%
					WHERE `event_participants_table`.`id` = %ID%;',
			],
			'event_decision_table' => [
				'event_decision_str' => 'SELECT CONCAT(
					`event_decision_table`.`decision_description`,\':\',
					`event_decision_table`.`decision_actor`,\':\',
					`event_decision_table`.`action_taken_with_date`,\':\',
					`event_decision_table`.`decision_status`,\':\',
					`event_decision_table`.`decision_status_remarks_by_superior`
					) FROM %TABLENAME%
					WHERE `event_decision_table`.`id` = %ID%;',
			],
			'meetings_table' => [
				'meetings_str' => 'SELECT CONCAT(
					`meetings_table`.`meeting_title`,\':\',
					`meetings_table`.`participants`,\':\',
					`meetings_table`.`venue`,\':\',
					`meetings_table`.`meeting_from_date`
					) FROM `meetings_table` WHERE `meetings_table` .`id` = %ID%;',
			],
			'meetings_agenda_table' => [
				'meetings_agenda_str' => 'SELECT CONCAT(
					`meetings_agenda_table`.`agenda_description`
					) FROM %TABLENAME%
					WHERE `meetings_agenda_table`.`id` = %ID%;',
			],
			'meetings_participants_table' => [
				'meetings_participants_str' => 'SELECT CONCAT(
					`meetings_participants_table`.`name`,\':\',
					`meetings_participants_table`.`designation`,\':\',
					`meetings_participants_table`.`participant_type`,\':\',
					`meetings_participants_table`.`accepted_status`,\':\',
					`meetings_participants_table`.`status_date`
					) FROM %TABLENAME%
					WHERE `meetings_participants_table`.`id` = %ID%;',
			],
			'meetings_decision_table' => [
				'meetings_decision_str' => 'SELECT CONCAT(
					`meetings_decision_table`.`decision_description`,\':\',
					`meetings_decision_table`.`decision_actor`,\':\',
					`meetings_decision_table`.`action_taken_with_date`,\':\',
					`meetings_decision_table`.`decision_status`
					) FROM %TABLENAME%
					WHERE `meetings_decision_table`.`id` = %ID%;',
			],
			'visiting_card_table' => [
				'visiting_card_str' => 'SELECT CONCAT(
					`visiting_card_table`.`name`,\':\',
					`visiting_card_table`.`recommended_by`,\':\',
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			...
			'event_outcomes_expected_table' => [
				'event_lookup' => 'SELECT `event_table`.`id`, `event_table`.`event_str` FROM `event_table` ORDER BY 2',
			],
			'event_participants_table' => [
				'event_lookup' => 'SELECT `event_table`.`id`, `event_table`.`event_str` FROM `event_table` ORDER BY 2',
			],
			'event_decision_table' => [
				'outcomes_expected_lookup' => 'SELECT `event_outcomes_expected_table`.`id`, `event_outcomes_expected_table`.`outcomes_expected_str` FROM `event_outcomes_expected_table` LEFT JOIN `event_table` as event_table1 ON `event_table1`.`id`=`event_outcomes_expected_table`.`event_lookup` ORDER BY 2',
				'decision_actor' => 'SELECT `user_table`.`id`, IF(CHAR_LENGTH(`user_table`.`memberID`) || CHAR_LENGTH(`user_table`.`name`), CONCAT_WS(\'\', `user_table`.`memberID`, \'::\', `user_table`.`name`), \'\') FROM `user_table` ORDER BY 2',
			],
			'meetings_table' => [
				'visiting_card_lookup' => 'SELECT `visiting_card_table`.`id`, `visiting_card_table`.`id` FROM `visiting_card_table` LEFT JOIN `user_table` as user_table1 ON `user_table1`.`id`=`visiting_card_table`.`given_by` ORDER BY 2',
				'event_lookup' => 'SELECT `event_table`.`id`, `event_table`.`id` FROM `event_table` ORDER BY 2',
			],
			'meetings_agenda_table' => [
				'meeting_lookup' => 'SELECT `meetings_table`.`id`, `meetings_table`.`meetings_str` FROM `meetings_table` LEFT JOIN `visiting_card_table` as visiting_card_table1 ON `visiting_card_table1`.`id`=`meetings_table`.`visiting_card_lookup` LEFT JOIN `event_table` as event_table1 ON `event_table1`.`id`=`meetings_table`.`event_lookup` ORDER BY 2',
			],
			'meetings_participants_table' => [
				'meeting_lookup' => 'SELECT `meetings_table`.`id`, `meetings_table`.`meetings_str` FROM `meetings_table` LEFT JOIN `visiting_card_table` as visiting_card_table1 ON `visiting_card_table1`.`id`=`meetings_table`.`visiting_card_lookup` LEFT JOIN `event_table` as event_table1 ON `event_table1`.`id`=`meetings_table`.`event_lookup` ORDER BY 2',
			],
			'meetings_decision_table' => [
				'agenda_lookup' => 'SELECT `meetings_agenda_table`.`id`, `meetings_agenda_table`.`meetings_agenda_str` FROM `meetings_agenda_table` LEFT JOIN `meetings_table` as meetings_table1 ON `meetings_table1`.`id`=`meetings_agenda_table`.`meeting_lookup` ORDER BY 2',
				'decision_actor' => 'SELECT `user_table`.`id`, IF(CHAR_LENGTH(`user_table`.`memberID`) || CHAR_LENGTH(`user_table`.`name`), CONCAT_WS(\'\', `user_table`.`memberID`, \'::\', `user_table`.`name`), \'\') FROM `user_table` ORDER BY 2',
			],
			'visiting_card_table' => [
				'given_by' => 'SELECT `user_table`.`id`, IF(CHAR_LENGTH(`user_table`.`memberID`) || CHAR_LENGTH(`user_table`.`name`), CONCAT_WS(\'\', `user_table`.`memberID`, \'::\', `user_table`.`name`), \'\') FROM `user_table` ORDER BY 2',
			],
		];
