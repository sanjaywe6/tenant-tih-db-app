...
<?php
	...
	if(is_array($arrTables) && count($arrTables)) {
		...
		foreach($tg as $tn => $tgroup) {
			...
			$tChkHL = array_search($tn, ['tenants','user_table','event_outcomes_expected_table','event_participants_table','event_decision_table','meetings_agenda_table','meetings_participants_table','meetings_decision_table']);
			/* allow homepage 'add new' for current table? */
			$tChkAHAN = array_search($tn, ['tenants','suggestion','event_table','event_outcomes_expected_table','event_participants_table','event_decision_table','meetings_table','meetings_agenda_table','meetings_participants_table','meetings_decision_table','visiting_card_table']);
