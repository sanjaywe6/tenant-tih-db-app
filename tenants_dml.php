...
function tenants_insert(&$error_message = '') {
	global $Translation;
	...
	$data = [
		'tenant_name' => Request::val('tenant_name', ''),
	...
	$recID = tableInsert('tenants', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		tenants_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function tenants_copy_children($destination_id, $source_id) {
	global $Translation;
...
function tenants_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	if(!check_record_permission('tenants', $selected_id, 'delete')) {
...
function tenants_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('tenants', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('tenants', $selected_id);
	...
	if(function_exists('tenants_before_update')) {
		...
		if(!tenants_before_update($data, getMemberInfo(), $args)) {
	...
	update_calc_fields('tenants', $data['selectedID'], calculated_fields()['tenants']);

	// hook: tenants_after_update
	if(function_exists('tenants_after_update')) {
		if($row = getRecord('tenants', $data['selectedID'])) $data = array_map('makeSafe', $row);
	...
	set_record_owner('tenants', $selected_id);
}

function tenants_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('tenants', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('tenants', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('tenants', $selectedId, 'view'))
	...
	if($hasSelectedId) {
		if(!($row = getRecord('tenants', $selectedId))) {
	...
	if(!$fieldsAreEditable) {
		$jsReadOnly = '';
		$jsReadOnly .= "\t\$j('#tenant_name').replaceWith('<div class=\"form-control-static\" id=\"tenant_name\">' + (\$j('#tenant_name').val() || '') + '</div>');\n";
	...
	$templateCode = str_replace('<%%UPLOADFILE(tenant_name)%%>', '', $templateCode);
	...
	if($hasSelectedId) {
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(tenant_name)%%>', safe_html($urow['tenant_name']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(tenant_name)%%>', html_attr($row['tenant_name']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(tenant_name)%%>', urlencode($urow['tenant_name']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(tenant_name)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(tenant_name)%%>', urlencode(''), $templateCode);
	...
	if($hasSelectedId) {
		$jdata = get_joined_record('tenants', $selectedId);
		if($jdata === false) $jdata = get_defaults('tenants');
		$rdata = $row;
	}
	$templateCode .= loadView('tenants-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: tenants_dv
	if(function_exists('tenants_dv')) {
		$args = [];
		tenants_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
